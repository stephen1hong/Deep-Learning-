...
def dErrors(X, y, y_hat):
    DErrorsDx1 = [-X[i][0]*(y[i]-y_hat[i]) for i in range(len(y))]# Fill in
    DErrorsDx2 = [-X[i][1]*(y[i]-y_hat[i]) for i in range(len(y))] # Fill in
    DErrorsDb = [-(y[i]-y_hat[i]) for i in range(len(y))]# Fill in
